...
% Last Modified by GUIDE v2.5 06-Jun-2017 15:50:02
...
% %             pnr(cnt)=max(pmax(find(cycle_nr==cnt-1,1),1));
% %             xnr(cnt)=max(xmax(find(cycle_nr==cnt-1,1),1));
% %             ynr(cnt)=max(ymax(find(cycle_nr==cnt-1,1),1));
% %         end
        summary={...
            'PARAMETERS','', '','','';...
            'Center',centerx,'um',centery,'um';...
            ...
            'Range Y',num2str(round(min(ypos)*1000)/1000),'um',num2str(round(max(ypos)*1000)/1000),'um';...
            'Time',num2str(floor(max(time)/60)),'min',num2str(ceil(max(time)-floor(max(time)/60)*60)),'sec';...
            'High power',num2str(round(mean(max_p)*1000)/1000),'uW ±',num2str(round((max(max_p)-min(max_p))*1000)/1000),'uW';...
            'Location X',num2str(round(mean(max_x)*1000)/1000),'um ±',num2str(round((max(max_x)-min(max_x))*1000)/1000),'um';...
            'Location Y',num2str(round(mean(max_y)*1000)/1000),'um ±',num2str(round((max(max_y)-min(max_y))*1000)/1000),'um'};
...
% --- Executes on button press in check_debug.
function check_debug_Callback(hObject, eventdata, handles)
% hObject    handle to check_debug (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of check_debug
